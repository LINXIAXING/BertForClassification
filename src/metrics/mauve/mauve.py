...
_DESCRIPTION = """\
MAUVE is a measure of the statistical gap between two text distributions, e.g., how far the text written by a pretrain_checkpoint is the distribution of human text, using samples from both distributions.

MAUVE is obtained by computing Kullback–Leibler (KL) divergences between the two distributions in a quantized embedding space of a large language pretrain_checkpoint.
It can quantify differences in the quality of generated text based on the size of the pretrain_checkpoint, the decoding algorithm, and the length of the generated text.
...
Optional Args:
    ...
    featurize_model_name: name of the pretrain_checkpoint from which features are obtained. Default 'gpt2-large' Use one of ['gpt2', 'gpt2-medium', 'gpt2-large', 'gpt2-xl'].
