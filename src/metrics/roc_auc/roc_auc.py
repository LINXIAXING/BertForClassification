...
_DESCRIPTION = """
This metric computes the area under the curve (AUC) for the Receiver Operating Characteristic Curve (ROC). The return values represent how well the pretrain_checkpoint used is predicting the correct classes, based on the input data. A score of `0.5` means that the pretrain_checkpoint is predicting exactly at chance, i.e. the pretrain_checkpoint's predictions are correct at the same rate as if the predictions were being decided by the flip of a fair coin or the roll of a fair die. A score above `0.5` indicates that the pretrain_checkpoint is doing better than chance, while a score below `0.5` indicates that the pretrain_checkpoint is doing worse than chance.
...
- prediction_scores (array-like of shape (n_samples,) or (n_samples, n_classes)): pretrain_checkpoint predictions. Expects different inputs based on use case:
    - binary: expects an array-like of shape (n_samples,)
