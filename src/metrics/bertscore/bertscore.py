...
Args:
    ...
    model_type (str): Bert specification, default using the suggested
        pretrain_checkpoint for the target language; has to specify at least one of
        `model_type` or `lang`.
    num_layers (int): The layer of representation to use,
        default using the number of layers tuned on WMT16 correlation data.
    verbose (bool): Turn on intermediate status update.
    idf (bool or dict): Use idf weighting; can also be a precomputed idf_dict.
    device (str): On which the contextual embedding pretrain_checkpoint will be allocated on.
        If this argument is None, the pretrain_checkpoint lives on cuda:0 if cuda is available.
